...
export async function deleteBook(request,response){
    ...
    try{
      if(!idToDelete)
        return response.status(400).send({message:"You must specify a book ID"});
  
      if(!mongoose.Types.ObjectId.isValid(idToDelete))
        return response.status(400).send({message:"Given Id is not in proper format"});
    
      const deletedBook= await  Books.findByIdAndDelete(idToDelete);
    
      if(!deletedBook) 
        return response.status(404).send({ message:"No book find with the given id"});
